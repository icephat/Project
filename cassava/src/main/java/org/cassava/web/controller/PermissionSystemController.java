...
import org.cassava.model.dto.PermissionSystemDTO;
...
public class PermissionSystemController {
	...
	@RequestMapping(value = {"/permissionsystem/index","/permissionsystem/","/permissionsystem"}, method = RequestMethod.GET)
	public String requesterIndex(Model model, Authentication authentication) {
		...
		List<Permission> permissionList = (List<Permission>) permissionService.findByStaffAndType(staff, "allOrganize");
		...
		for (Permission permission : permissionList) {
			...
			List<Integer> organizationIds = permissionOrganizationService.findOrganizationIdByPermission(permission);

			List<Object> obj2 = targetofsurveyService
					.findCountSurveyAndTargetOfSurveyByTargetOfSurveyIdAndDateAndProvinceAndStatusByListOrganization(
							tosIds, permission.getDateInfoStart(), permission.getDateInfoEnd(), proviceIds, "Complete",
							organizationIds);
		...
		return "permissionsystem/index";
	}

	@RequestMapping(value = "/permissionsystem/create", method = RequestMethod.POST)
	public ResponseEntity<Response<ObjectNode>> addPermission(
			@ModelAttribute("permissionDTO") PermissionSystemDTO permissionDTO, Model model,
			Authentication authentication) {
		ResponseEntity<Response<ObjectNode>> responseEntity = validate(permissionDTO);
		if (responseEntity.getStatusCode() == HttpStatus.OK) {
			...
			permission.setType("allOrganize");
			...
			for (int id : permissionDTO.getOrganizations()) {
				if (id != 0) {
					Organization organization = organizationService.findById(id);
					System.out.println(organization.getName());

					PermissionOrganization permissionOrganization = new PermissionOrganization(organization,
							permission);
					permissionOrganizationService.save(permissionOrganization);
				}
			}
		}
		// return "redirect:/permissionsystem/index";
		return responseEntity;
	}

	private ResponseEntity<Response<ObjectNode>> validate(PermissionSystemDTO permissionDTO) {
...
//		}
		...
		// Set<ConstraintViolation<Planting>> violations =
		// validator.validate(plantingDTO.getPlanting());
...
//		}	
	...
	@RequestMapping(value = { "/permissionsystem/request/delete/{permisisonId}",
			"/permissionsystem/request/delete/{permisisonId}/page/{toPage}" }, method = RequestMethod.GET)
...
// model.addAttribute(permission);
		...
		if (toPage.equals("approved"))
			return "redirect:/permissionsystem/request/approved";
		else
			return "redirect:/permissionsystem/index";

	}

	@RequestMapping(value = "/permissionsystem/request/download/{permisisonfileId}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/permissionsystem/request", method = RequestMethod.GET)
	public String requesterApproveIndex(Model model, Authentication authentication) {
		...
		List<Permission> permissions = permissionService.findByListStatusAndType(status, "allOrganize");
		...
		for (Permission permission : permissions) {
			...
			targetOfSurveyAndCounts.add(targetofsurveyService
					.findCountSurveyAndTargetOfSurveyByTargetOfSurveyIdAndDateAndProvinceAndStatusByListOrganization(
							tosIds, permission.getDateInfoStart(), permission.getDateInfoEnd(), provinceIds, "Complete",
							organizationIds));
		}
		...
		model.addAttribute("formatter", formatter);
		return "permissionsystem/requestIndex";

	}

	@RequestMapping(value = "/permissionsystem/request/approved", method = RequestMethod.GET)
	public String requesterApproved(Model model, Authentication authentication) {
		...
		model.addAttribute("provinces", provinces);
		model.addAttribute("formatter", formatter);
		return "permissionsystem/requestApproved";

	}

	@RequestMapping(value = "/permissionsystem/approve/{permisisonId}", method = RequestMethod.GET)
	public String approvePermission(Model model, @PathVariable("permisisonId") int permissionId,
		...
		return "redirect:/permissionsystem/request";
	}

	@RequestMapping(value = "/permissionsystem/reject/{permisisonId}", method = RequestMethod.GET)
