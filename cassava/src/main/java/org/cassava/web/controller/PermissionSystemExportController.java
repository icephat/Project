...
@Controller
public class PermissionSystemExportController {
	...
	@RequestMapping(value = {"/permissionexport/system/index","/permissionexport/system/","/permissionexport/system"}, method = RequestMethod.GET)
	public String Index(Model model, Authentication authentication) {
		Staff staff = staffService.findByUserName(MvcHelper.getUsername(authentication));
		List<Request> requests = requestService.findByUserIdAndType(staff.getStaffId(), "allOrganization");
		model.addAttribute("requests", requests);
		Format formatter = new SimpleDateFormat("d MMM yyyy", new Locale("th", "TH"));
		model.addAttribute("formatter", formatter);
		model.addAttribute("toPage", "index");

		return "/permissionsystemexport/index";
	}

	@RequestMapping(value = "/permissionexport/system/permissions", method = RequestMethod.GET)
	public String permission(Model model, Authentication authentication) {
		...
		model.addAttribute("formatter", formatter);
		return "/permissionsystemexport/permission";
	}
	@RequestMapping(value = { "/permissionexport/system/permission/{pid}",
			"/permissionexport/system/permission/{pid}/{search}" }, method = RequestMethod.POST)
	public String Index(@PathVariable("pid") int id, Model model, Authentication authentication,
			...
			@RequestParam(value = "organization", required = false) List<Integer> organizationid) {
		Permission permission = permissionService.findById(id);
		if(permission == null) {
			return "redirect:/permissionexport/system/";
		}
		...
		if (search != null && search.equals("search")) {
			...
			List<SurveyTarget> serveyTargetSearch = surveyTargetService
					.findByTargetOfSurveysAndBetweenDateAndProvincesAndStatusAndFieldNameAndOrganization(toss,
		...
		model.addAttribute("listorganization", organizationService.findBySurveyTarget(serveyTarget));

		Format formatter = new SimpleDateFormat("d MMM yyyy", new Locale("th", "TH"));
		model.addAttribute("formatter", formatter);
		return "/permissionsystemexport/permissionRequest";
	}

	@RequestMapping(value = "/permissionexport/system/requestSave", method = RequestMethod.POST)
	public String save(@RequestParam("listsurveyid") List<Integer> listsurveyid, Model model,
		...
		return "redirect:/permissionexport/system/index";
	}

	@RequestMapping(value = "/permissionexport/system/requestDelete/{id}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/permissionexport/system/requestDetail/{id}", method = RequestMethod.POST)
	public String RequestField(Model model, Authentication authentication, @PathVariable("id") int reid,
		...
		return "/permissionsystemexport/requestdetail";
	}

	@RequestMapping(value = {"/permissionexport/system/approve/index","/permissionexport/system/approve/","/permissionexport/system/approve"}, method = RequestMethod.GET)
	public String IndexApprove(Model model, Authentication authentication) {
		Staff staff = staffService.findByUserName(MvcHelper.getUsername(authentication));
		List<String> organization = new ArrayList<String>();
		organization.add("กรมส่งเสริมการเกษตร");
		organization.add("กรมวิชาการเกษตร");
		organization.add("ภาควิชาโรคพืช มก. กพส.");
		ArrayList<String> status = new ArrayList<String>();
		ArrayList<String> statusre = new ArrayList<String>();
		List<Request> requests = new ArrayList<Request>();
		status.add("Waiting");
		statusre.add("Waiting");

		Format formatter = new SimpleDateFormat("d MMM yyyy", new Locale("th", "TH"));
		model.addAttribute("formatter", formatter);
		model.addAttribute("toPage", "approveindex");
		model.addAttribute("staff", staff);
		model.addAttribute("organization", organization);
		if (staff.getOrganization().getName().equals("กรมส่งเสริมการเกษตร")) {
			requests = requestService.findByStatusDaeAndOrganizationAndTypeAndStatusRequest(status, "allOrganization",
					statusre);
		} else if (staff.getOrganization().getName().equals("กรมวิชาการเกษตร")) {
			requests = requestService.findByStatusDaoAndOrganizationAndType(status, "allOrganization", statusre);
		} else if (staff.getOrganization().getName().equals("ภาควิชาโรคพืช มก. กพส.")) {
			requests = requestService.findByStatusDppathoAndOrganizationAndType(status, "allOrganization", statusre);
		} else {
			requests = requestService.findByStatus(statusre, "allOrganization");
			model.addAttribute("requests", requests);
			model.addAttribute("toPage", statusre.get(0));
			return "/permissionsystemexport/requestApproved";
		}
		model.addAttribute("requests", requests);

		return "/permissionsystemexport/approveindex";
	}

	@RequestMapping(value = "/permissionexport/system/requestApproved", method = RequestMethod.POST)
	public String requestApproved(Model model, Authentication authentication,
		...
		if (status.equals("Approved")) {
			...
			restatus.add("Waiting");
		}
		List<String> organization = new ArrayList<String>();
		organization.add("กรมส่งเสริมการเกษตร");
		organization.add("กรมวิชาการเกษตร");
		organization.add("ภาควิชาโรคพืช มก. กพส.");
		requests = requestService.findByStatus(restatus, "allOrganization");
		model.addAttribute("requests", requests);
		model.addAttribute("requests", requests);
		Format formatter = new SimpleDateFormat("d MMM yyyy", new Locale("th", "TH"));
		model.addAttribute("formatter", formatter);
		model.addAttribute("toPage", status);
		model.addAttribute("staff", staff);
		return "/permissionsystemexport/requestApproved";
	}

	@RequestMapping(value = "/permissionexport/system/requestReject/{id}/{organization}", method = RequestMethod.GET)
	public String RequestReject(Model model, Authentication authentication, @PathVariable("id") int reid,
		...
		if(re == null) {
			return "redirect:/permissionexport/system/approve/index";
		}
	...
	@RequestMapping(value = "/permissionexport/system/requestApprove/{id}/{organization}", method = RequestMethod.GET)
	public String RequestApprove(Model model, Authentication authentication, @PathVariable("id") int reid,
		...
		Request re = requestService.findById(reid);
		if (organizationcheck.get(0).equals(organization)) {
			re.setStatusDae("Approve");
			re.setStaffByApproveByDae(staff);
		} else if (organizationcheck.get(1).equals(organization)) {
			re.setStatusDa("Approve");
			re.setStaffByApproveByDa(staff);
		} else if (organizationcheck.get(2).equals(organization)) {
			re.setStatusDppatho("Approve");
			re.setStaffByApproveByDpphato(staff);
		}
		List<String> status = new ArrayList<String>();
		status.add(re.getStatusDae());
	...
	@RequestMapping(value = "/permissionexport/system/export", method = RequestMethod.GET)
	public String Exportindex(Model model, Authentication authentication) {
		int userid = userService.findByUsername(MvcHelper.getUsername(authentication)).getUserId();
		List<Object> listreque = requestdetailService.findByUserId(userid);
		Format formatter = new SimpleDateFormat("yyyy-MM-dd");
		List<String> date = new ArrayList<String>();
		for (Object object : listreque) {
			Object[] ob = (Object[]) object;
			RequestDetail rd = (RequestDetail) ob[0];
			date.add(formatter.format(rd.getRequest().getDateRequest()));
		}
		model.addAttribute("date", date);
		model.addAttribute("listrequest", listreque);
		return "/permissionsystemexport/exportsurvey";
	}

	@RequestMapping(value = "/permissionexport/system/export/{id}", method = RequestMethod.GET)
